...
type RecallDailyApi struct {
	logger *logger.Logger
	alarm  message.Alarm
}

func NewRecallDailyApi() *RecallDailyApi {
	log := local.Logger.WithContext(context.WithValue(local.Ctx, logger.ModuleKey, local.Module.Add(".RecallDailyApi")))
	return &RecallDailyApi{
		logger: log,
...
func (api *RecallDailyApi) Generate(ctx *gin.Context) {
	...
	svc := service.NewRecallDailySvc(ctx, local.CMSDB, local.CenterDB, api.logger)
	go func() {
...
func (api *RecallDailyApi) List(ctx *gin.Context) {
...
func (api *RecallDailyApi) Export(ctx *gin.Context) {
