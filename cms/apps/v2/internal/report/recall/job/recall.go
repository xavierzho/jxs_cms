...
func NewRecallJob() *RecallJob {
	// 设置日志模块和上下文
	log := local.JobWorkerLogger.WithContext(context.WithValue(local.JobWorkerLogger.Context, logger.ModuleKey, local.JobWorkerLogger.ModuleKey().Add(".RecallJob")))
	ctx := &gin.Context{
...
// Name 返回作业的名称
func (*RecallJob) Name() string {
	return "RecallJob"
}

// Run 将当前作业实例添加到作业队列中
func (j *RecallJob) Run() {
	local.JobWorker.AddJobToQueue(j.Name())
}

// Work 执行作业的具体逻辑
func (j *RecallJob) Work() {
	// 获取前一天的日期字符串
	cDateStr := time.Now().AddDate(0, 0, -1).Format(pkg.DATE_FORMAT)
	// 初始化成本奖励服务
	svc := service.NewRecallSvc(j.ctx, local.CMSDB, local.CenterDB, j.logger)
	if err := svc.Generate(&form.GenerateRequest{DateRangeRequest: iForm.DateRangeRequest{DateRange: [2]string{cDateStr, cDateStr}}}); err != nil {
		j.alarm.AlertErrorMsg(fmt.Sprintf("RecallSvc.Generate error: %v", err), message.CmsId)
	}
}
