...
func NewInviteBetDailyJob() *InviteBetDailyJob {
	// 设置日志模块和上下文
	log := local.JobWorkerLogger.WithContext(context.WithValue(local.JobWorkerLogger.Context, logger.ModuleKey, local.JobWorkerLogger.ModuleKey().Add(".InviteBetDailyJob")))
	ctx := &gin.Context{
		Request: &http.Request{},
	}
	ctx.Request = ctx.Request.WithContext(log.Context)
	return &InviteBetDailyJob{
		ctx:    ctx,
		logger: log,
...
// Name 返回作业的名称
func (*InviteBetDailyJob) Name() string {
	return "InviteBetDailyJob"
}

// Run 将当前作业实例添加到作业队列中
func (j *InviteBetDailyJob) Run() {
	local.JobWorker.AddJobToQueue(j.Name())
}

// Work 执行作业的具体逻辑
func (j *InviteBetDailyJob) Work() {
	// 获取前一天的日期字符串
	cDateStr := time.Now().AddDate(0, 0, -1).Format(pkg.DATE_FORMAT)
	// 初始化成本奖励服务
	svc := service.NewInviteBetDailySvc(j.ctx, local.CMSDB, local.CenterDB, j.logger)
	if err := svc.Generate(&form.GenerateDailyRequest{DateRangeRequest: iForm.DateRangeRequest{DateRange: [2]string{cDateStr, cDateStr}}}); err != nil {
		j.alarm.AlertErrorMsg(fmt.Sprintf("InviteBetSvc.Generate error: %v", err), message.CmsId)
	}
}
