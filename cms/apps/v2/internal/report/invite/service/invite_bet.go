...
type InviteBetSvc struct {
	logger  *logger.Logger
	dao     *dao.InviteBetDao
	userDao *cDao.UserDao
}

func NewInviteBetSvc(ctx *gin.Context, engine, center *gorm.DB, log *logger.Logger) *InviteBetSvc {
	log = log.WithContext(context.WithValue(ctx, logger.ModuleKey, log.ModuleKey().Add(".InviteBetSvc")))
	return &InviteBetSvc{
		logger:  log,
		dao:     dao.NewInviteBetDao(engine, center, log),
		userDao: cDao.NewUserDao(center, log), //注入user dao访问对象
	}
}

func (svc *InviteBetSvc) Generate(params *form.GenerateRequest) (e *errcode.Error) {
...
func (svc *InviteBetSvc) generate(cDate time.Time, queryParams database.QueryWhereGroup) (err error) {
...
func (svc *InviteBetSvc) _generate(cDate time.Time, queryParams database.QueryWhereGroup) (data []*dao.InviteBet, err error) {
...
// 获取user信息
func (svc *InviteBetSvc) userInfo(params *form.AllRequest, queryParams database.QueryWhereGroup) (listUserCondition, generateUserCondition database.QueryWhereGroup, err error) {
...
func (svc *InviteBetSvc) List(params *form.ListRequest) (summary map[string]any, data []*form.InviteBet, e *errcode.Error) {
...
func (svc *InviteBetSvc) Export(params *form.AllRequest) (data *excel.Excel[*form.InviteBet], e *errcode.Error) {
	dateRange, queryParams, err := params.Parse()
	if err != nil {
