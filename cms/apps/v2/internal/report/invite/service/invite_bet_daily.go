...
type InviteBetDailySvc struct {
	logger  *logger.Logger
	dao     *dao.InviteBetDailyDao
	userDao *cDao.UserDao
}

func NewInviteBetDailySvc(ctx *gin.Context, engine, center *gorm.DB, log *logger.Logger) *InviteBetDailySvc {
	log = log.WithContext(context.WithValue(ctx, logger.ModuleKey, log.ModuleKey().Add(".InviteBetDailySvc")))
	return &InviteBetDailySvc{
		logger:  log,
		dao:     dao.NewInviteBetDailyDao(engine, center, log),
		userDao: cDao.NewUserDao(center, log),
	}
}

func (svc *InviteBetDailySvc) Generate(params *form.GenerateDailyRequest) (e *errcode.Error) {
...
func (svc *InviteBetDailySvc) generate(cDate time.Time) (err error) {
...
func (svc *InviteBetDailySvc) List(params *form.ListDailyRequest) (summary map[string]any, data []*form.InviteBetDaily, e *errcode.Error) {
	dateRange, err := params.Parse()
	if err != nil {
...
func (svc *InviteBetDailySvc) Export(params *form.AllDailyRequest) (data *excel.Excel[*form.InviteBetDaily], e *errcode.Error) {
	dateRange, err := params.Parse()
	if err != nil {
