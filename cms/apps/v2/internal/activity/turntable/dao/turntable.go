...
type Turntable struct {
	...
	PointType  int64  `gorm:"column:point_type; type:bigint" json:"point_type"`   //抽奖支付类型  10 现金点  11 欧气值
...
func (d *TurntableDao) all(dateRange [2]time.Time, queryParams database.QueryWhereGroup) *gorm.DB {
	return d.center.
		Select(
			...
			"apwc.point_type as  point_type", //抽奖资源类型
			"apwc.point as point",            //抽奖消耗
			"i.inner_price as prize_value",   //奖品价值
		).
		Table("activity_prize_wheel_history apwh").
		Joins("left join users AS u ON apwh.user_id = u.id").
		Joins("left join activity_prize_wheel_config AS apwc ON apwh.config_id = apwc.id").
		Joins("left join activity_prize_wheel_award_config AS apwac ON apwh.award_id = apwac.id").
