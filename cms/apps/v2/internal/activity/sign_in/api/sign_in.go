...
import (
	...
	"data_backend/apps/v2/internal/activity/sign_in/service"
...
func NewSignInApi() *SignInApi {
	log := local.Logger.WithContext(context.WithValue(local.Ctx, logger.ModuleKey, local.Module.Add(".SignInApi")))
	return &SignInApi{
		logger: log,
...
func (api *SignInApi) List(ctx *gin.Context) {
	params := &form.Request{}
	...
	svc := service.NewSignInSvc(ctx, local.CenterDB, api.logger)
...
func (api *SignInApi) Export(ctx *gin.Context) {
