...
import (
	...
	"data_backend/apps/v2/internal/inquire/recall/api"
...
func InitRouter(r *gin.RouterGroup) (err error) {
	{
		rg := r.Group("recall")
		RecallApi := api.NewRecallApi() // 初始化InviteRecApi实例，用于处理邀请的API请求
		rg.GET("", local.PermissionGate.CheckPerm("inquire_recall_view"), RecallApi.List)
		rg.POST("/export", local.PermissionGate.CheckPerm("inquire_recall_view"), RecallApi.Export)
	}
