...
func NewRecallDao(center *gorm.DB, log *logger.Logger) *RecallDao {
...
func (d *RecallDao) ListAndSummary(summaryField []string, dateRange [2]time.Time, queryParams database.QueryWhereGroup, pager app.Pager) (summary map[string]any, data []*RecallSvc, err error) {
...
func (d *RecallDao) All(dateRange [2]time.Time, queryParams database.QueryWhereGroup) (data []*RecallSvc, err error) {
...
func (d *RecallDao) all(dateRange [2]time.Time, queryParams database.QueryWhereGroup) *gorm.DB {
	return d.center.
		Select(
