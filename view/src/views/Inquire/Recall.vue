    ...
    <template v-slot:table-search>
          ...
          <!-- <el-form-item :label="$t('user.IsRewards')" @click="() => {fetchDaily({page: 1});}" ><el-switch v-model="searchForm.is_rewards" /></el-form-item> -->
            ...
            <template v-slot:table-column>
                ...
                <el-table-column prop="user_id" :label="$t('inquire.recall.user_id')" min-width="90px" align="center">
                  <template v-slot="data">{{ data.row.user_id }}</template>
                </el-table-column>
                <el-table-column prop="user_name" :label="$t('inquire.recall.user_name')" min-width="90px" align="center">
                  <template v-slot="data">{{ data.row.user_name}}</template>
                </el-table-column>
                <el-table-column prop="parent_user_id" :label="$t('inquire.recall.parent_user_id')" min-width="90px" align="center">
                  <template v-slot="data">{{ data.row.parent_user_id }}</template>
                </el-table-column>
                <el-table-column prop="parent_user_name" :label="$t('inquire.recall.parent_user_name')" min-width="90px" align="center">
  ...
  export default {
    ...
    methods:{
      ...
      fetch(params = {}) {
        ...
        api.getInquireRecall(this.searchForm).then(res => {
      ...
      fetchExport(params = {}){
        this.loading = true
        this.searchForm = Object.assign({}, this.searchForm, params)
        api.exportInquireRecall(this.searchForm).
        finally(() => {
          this.loading = false
        })
      },
