  ...
  <div>
    ...
    >
      <template v-slot:table-search>
        ...
        <el-form inline v-model="searchFormLog">
          <el-form-item :label="$t('inquire.task.task_type')">
            <el-select v-model="searchFormLog.task_type_list" multiple clearable>
              <el-option v-for="item in inquireTaskType" :label="item.label" :value="item.value"
                         :key="item.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :label="$t('inquire.task.task_key')">
            <el-select v-model="searchFormLog.task_key_list" multiple clearable>
              <el-option v-for="item in inquireTaskKey" :label="item.label" :value="item.value"
                         :key="item.value"></el-option>
            </el-select>
          </el-form-item>
        </el-form>
      ...
      <template v-slot:table-column>
        ...
        <el-table-column prop="task_type_str" :label="$t('inquire.task.task_type')" min-width="90px"
                         align="center"></el-table-column>
        <el-table-column prop="task_key_str" :label="$t('inquire.task.task_key')" min-width="90px"
                         align="center"></el-table-column>
        <el-table-column prop="task_name" :label="$t('inquire.task.task_name')" min-width="90px"
                         align="center"></el-table-column>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      inquireTaskType: 'option/inquireTaskType',
      inquireTaskKey: 'option/inquireTaskKey',
    }),
  },
  created() {
    if (this.inquireTaskType.length === 0) {
      this.$store.dispatch('option/update', 'inquireTaskType')
      console.log(this.inquireTaskType)
    }
    if (this.inquireTaskKey.length === 0) {
      this.$store.dispatch('option/update', 'inquireTaskKey')
      console.log(this.inquireTaskType)
    }
  ...
  methods: {
    fetch(params = {}) {
      ...
      api.getInquireTask(searchFormLog).then(res => {
    ...
    fetchExport(params = {}) {
      ...
      this.searchFormLog = Object.assign(this.searchFormLog, params)
      let searchFormLog = Object.assign({}, this.searchFormLog)
      api.exportInquireTaskList(searchFormLog).finally(() => {
        this.loading = false
      })
    },
    fetchDetail(params = {}, row) {
      ...
      api.getInquireTaskAwardDetail(this.searchFormDetail).then(res => {
        row.detail = res.data || []
      }).catch(() => {
        row.detail = []
      }).finally(() => {
    ...
    fetchExportDetail(params = {}) {
      ...
      this.searchFormLog = Object.assign(this.searchFormLog, params)
      let searchFormLog = Object.assign({}, this.searchFormLog)
      api.exportInquireTaskAwardDetail(searchFormLog).finally(() => {
        this.loading = false
      })
    },
    expandChange(row) {
      if (row.detail === undefined) {
        this.fetchDetail({task_id: row.task_id,}, row)
      }
    },
    summaryMethod({columns, data}) {
      return [
        'Total', undefined, undefined, undefined, undefined,
        undefined, Number(this.summary["reward_value_item"]).toLocaleString(),
