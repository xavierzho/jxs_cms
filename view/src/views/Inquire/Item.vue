      ...
      <template v-slot:table-search>
        ...
        <el-form inline v-model="searchFormLog">
          <el-form-item :label="$t('inquire.item.log_type')">
            <el-select v-model="searchFormLog.log_type_list" multiple clearable>
              <el-option v-for="item in inquireItemLogType" :label="item.label" :value="item.value" :key="item.value" ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <span slot="label">{{$t('inquire.item.gacha_name')}}
              <el-tooltip effect="dark" :content="$t('inquire.item.filterOptionTip')" placement="top"><i class='el-icon-info' /></el-tooltip>
            </span>
        ...
        <el-form inline v-model="searchFormLog">
          ...
          <el-form-item :label="$t('inquire.item.show_price')">
            <input-number-range v-model="searchFormLog.show_price_range" clearable :range="[0, 2**31-1]"></input-number-range>
          </el-form-item>
          <el-form-item :label="$t('inquire.item.inner_price')">
            <input-number-range v-model="searchFormLog.inner_price_range" clearable :range="[0, 2**31-1]"></input-number-range>
          </el-form-item>
          <el-form-item>
            ...
            <el-button type="warning" v-loading="loading" @click="fetchExportDetail({})">{{ $t('inquire.item.export_detail') }}</el-button>
          </el-form-item>
        </el-form>
      </template>
      <template v-slot:table-column>
        ...
        <el-table-column prop="bet_nums" :label="$t('inquire.item.bet_nums')" min-width="90px" align="center">
          <template v-slot="data">
            <template v-if="data.row.log_type<=199 && data.row.level_type === 1">{{data.row.bet_nums | localeNum}}</template>
        ...
        <el-table-column type="expand">
          <template v-slot="record">
            <template v-if="(record.row.log_type===200)&& !loading">
              <el-row :gutter="20">
                <el-col :span="12">
                  <item-detail :loading="loading" :data="record.row.detail[0]" :span-method="spanMethodDetailMarket(record.row.detail[0])">
                    <template v-slot:table-top-column>
                      <el-table-column prop="user_id" :label="$t('user.UserID')" min-width="90px" align="center"></el-table-column>
                      <el-table-column prop="user_name" :label="$t('inquire.item.creator')" min-width="90px" align="center"></el-table-column>
                      <el-table-column prop="amount" :label="$t('inquire.item.ask_price')" min-width="90px" align="center">
                ...
                <el-col :span="12">
                  <item-detail :loading="loading" :data="record.row.detail[1]" :span-method="spanMethodDetailMarket(record.row.detail[1])">
                    <template v-slot:table-top-column>
                      <el-table-column prop="user_id" :label="$t('user.UserID')" min-width="90px" align="center"></el-table-column>
                      <el-table-column prop="user_name" :label="$t('inquire.item.offerer')" min-width="90px" align="center"></el-table-column>
                      <el-table-column prop="amount" :label="$t('inquire.item.offer_price')" min-width="90px" align="center">
                        <template v-slot="detail">{{detail.row.amount | localeNum2f}}</template>
                      </el-table-column>
                    </template>
                    <template v-slot:table-tail-column>
            ...
            <template v-else-if="(record.row.log_type===100002 || record.row.log_type===100003) && !loading">
            ...
            <template v-else-if="!loading">
              <item-detail :loading="loading" :data="record.row.detail" show-level-name :span-method="spanMethodDetailBet(record.row.detail)">
                <template v-slot:table-top-column>
...
export default {
  ...
  methods:{
    fetch(params = {}) {
      ...
      api.getInquireItemLog(searchFormLog).
      then(res => {
    ...
    fetchExport(params = {}){
      ...
      searchFormLog.show_price_range = this.setRangeBorder(searchFormLog.show_price_range)
      searchFormLog.inner_price_range = this.setRangeBorder(searchFormLog.inner_price_range)
      api.exportInquireItemLog(searchFormLog).
      finally(() => {
        this.loading = false
      })
    },
    fetchDetail(params = {}, row){
      ...
      api.getInquireItemDetail(this.searchFormDetail).
      then(res => {
        row.detail = res.data || []
      }).catch(()=>{
        row.detail = []
    ...
    fetchExportDetail(params = {}){
      ...
      searchFormLog.show_price_range = this.setRangeBorder(searchFormLog.show_price_range)
      searchFormLog.inner_price_range = this.setRangeBorder(searchFormLog.inner_price_range)
      api.exportInquireItemDetailLog(searchFormLog).
      finally(() => {
        this.loading = false
      })
    },
