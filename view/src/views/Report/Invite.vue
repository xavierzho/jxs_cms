    ...
    <search-table
      ...
      <template v-slot:table-search>
        <el-form inline v-model="searchForm">
          ...
          <el-form-item :label="$t('report.invite.is_daily')">
      ...
      <template v-slot:table-column>
        <template v-if="!searchForm.is_daily">
          ...
          <el-table-column prop="user_id" :label="$t('report.invite.user_id')" min-width="90px" align="center">
            <template v-slot="data">{{ data.row.user_id }}</template>
          </el-table-column>
          <el-table-column prop="user_name" :label="$t('report.invite.user_name')" min-width="90px" align="center">
            <template v-slot="data">{{ data.row.user_name }}</template>
          </el-table-column>
          <el-table-column prop="parent_user_id" :label="$t('report.invite.parent_user_id')" min-width="90px" align="center">
            <template v-slot="data">{{ data.row.parent_user_id }}</template>
          </el-table-column>
          <el-table-column prop="parent_user_name" :label="$t('report.invite.parent_user_name')" min-width="90px" align="center">
            <template v-slot="data">{{ data.row.parent_user_name }}</template>
          </el-table-column>
          <el-table-column prop="amount" :label="$t('report.invite.amount')" min-width="90px" align="center">
            <template v-slot="data">{{ data.row.amount | localeNum2f }}</template>
          </el-table-column>
        </template>
        <template v-else>
          <el-table-column prop="date" :label="$t('common.Date')" min-width="90px" align="center"></el-table-column>
          <el-table-column prop="total_amount" :label="$t('report.invite.daily.total_amount')" min-width="90px" align="center">
            <template v-slot="data">{{ data.row.total_amount | localeNum2f }}</template>
          </el-table-column>
          <el-table-column prop="amount" :label="$t('report.invite.amount')" min-width="90px" align="center">
            <template v-slot="data">{{ data.row.amount | localeNum2f }}</template>
          </el-table-column>
          <el-table-column prop="difference" :label="$t('report.invite.daily.difference')" min-width="90px" align="center">
  ...
  methods:{
    ...
    fetch(params = {}) {
      ...
      if (this.searchForm.is_daily) {
          api.getReportInviteBetDaily(this.searchForm).then(res => {
          ...
          api.getReportInviteBet(this.searchForm).then(res => {
          this.tableData = res.data || []
          this.summary = res.headers.summary || {}
          this.total = this.summary['total'] || 0
          }).catch(()=>{
    ...
    fetchExport(params = {}){
      this.loading = true
      this.searchForm = Object.assign({}, this.searchForm, params)
      api.exportReportInviteBet(this.searchForm).
      finally(() => {
        this.loading = false
      })
    },
    fetchDailyExport(params = {}){
      this.loading = true
      this.searchForm = Object.assign({}, this.searchForm, params)
      api.exportReportInviteBetDaily(this.searchForm).
      finally(() => {
        this.loading = false
      })
    },
    summaryMethod({ columns, data }){
      return [
        ...
        Number(this.summary['amount'] || 0).toLocaleString(),
      ]
    },
